...
module Impl_Adder_C(CB:Callbacks) =
  struct
    type nonrec comms =
      {
      send_int: int -> unit ;
      ...
      let open CB in
        ...
        and run_state_4 env =
          ...
          | "sum" ->
              ...
              let env = state4Receivesum env payload in run_state_1 env
          | _ -> assert false
        and run_state_6 env =
          let comms = router `S in
          let label = comms.recv_string () in
          ...
          | "bye" ->
              ...
              let env = state6Receivebye env payload in run_state_7 env
          | _ -> assert false
