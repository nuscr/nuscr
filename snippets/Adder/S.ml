...
module Impl_Adder_S(CB:Callbacks) =
  struct
    type nonrec comms =
      {
      send_int: int -> unit ;
      ...
      let open CB in
        let rec run_state_1 env =
          ...
          | "bye" ->
              ...
              let env = state1Receivebye env payload in run_state_6 env
          | "add" ->
              let payload = comms.recv_int () in
              let env = state1Receiveadd env payload in run_state_3 env
          | _ -> assert false
        and run_state_3 env =
          let comms = router `C in
          let label = comms.recv_string () in
          ...
          | "add" ->
              let payload = comms.recv_int () in
              let env = state3Receiveadd env payload in run_state_4 env
          | _ -> assert false
        and run_state_4 env =
          let comms = router `C in
          match state4Send env with
          | (env, `sum payload) ->
              (comms.send_string "sum";
               comms.send_int payload;
               run_state_1 env)
        and run_state_6 env =
          let comms = router `C in
        ...
        and run_state_7 env = env in run_state_1 env
  end
