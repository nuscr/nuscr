...
let equal_coinductive lty1 lty2 =
  ...
  let rec aux lty1 lty2 =
    if Hash_set.mem memory (lty1, lty2) then true
      ...
      | TVarL (_, [], lty'), lty when Lazy.is_val lty' ->
          aux (Lazy.force lty') lty
      | lty, TVarL (_, [], lty') when Lazy.is_val lty' ->
          aux (Lazy.force lty') lty
      | MuL (tv1, rvs1, lty1'), MuL (tv2, rvs2, lty2') ->
...
let rec really_merge unguarded_tv projected_role lty1 lty2 =
  ...
  | MuL (tv1, [], lty1'), MuL (tv2, [], lty2')
    when TypeVariableName.equal tv1 tv2 ->
      let unguarded_tv = Set.add unguarded_tv tv1 in
...
let rec project' env (projected_role : RoleName.t) =
  ...
  | ChoiceG (choice_r, g_types) -> (
      ...
      | _ -> (
        match
          List.reduce
            ~f:(merge ~unguarded_tv:env.unguarded_tv projected_role)
            l_types
        with
        | Some l -> l
        | None -> EndL ) )
