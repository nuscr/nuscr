...
let filter z x = match x with
  | SendL (m,n,l) ->
    let lab = let open Gtype in m.label in
    if String.equal (LabelName.user lab) "crash"
      then z
      else SendL (m,n,l) :: z
  | _ -> x :: z
  ...
  let rec conv_ltype_aux env =
    ...
    | (RecvL (m, n, l) | SendL (m, n, l)) as curr_ty ->
        ...
        let action =
          ...
          | RecvL _ ->
            let lab = let open Gtype in m.label in
            if (Pragma.error_handling_crash_branch ()) && 
                          String.equal (LabelName.user lab) "crash"
            then
              let open Gtype in
              let fresh : message = {label = (LabelName.of_string "#"); payload = []} in
        ...
        ({env with g}, curr)
    | ChoiceL (_r, ltys) ->
        let ltys = if Pragma.error_handling_crash_branch ()
          then List.fold ltys ~init:[] ~f:filter
          else ltys
        in
        let curr = fresh () in
